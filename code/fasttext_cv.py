...
def get_model():
    model = Sequential()
    model.add(Embedding(nb_words, EMBEDDING_DIM, input_length=MAX_SEQUENCE_LENGTH, weights=[embedding_matrix],
                        trainable=True))
...
for ii,(idx_train, idx_val) in enumerate(skf):
    ...
    history = model.fit(X_train_tr, y_tr, batch_size=1024, epochs=60, verbose=1, validation_data=(X_train_te,y_te),
                        #callbacks=[early_stop, check_point]
    )

    #model.load_weights('cate_model.hdf5')
    preds_te = model.predict_proba(X_train_te)
...
with open("score_cv.txt", "a") as f:
    f.write("%s now score is:" % "fasttext" + str(score_cv_list) + "\n")
...
train_prob.to_csv('../sub_prob/train_prob_fasttext_cv_%s.csv'%score,index=None)
#保存预测概率文件
test_prob=pd.DataFrame(test_pred)
test_prob.columns=["class_prob_%s"%i for i in range(1,test_pred.shape[1]+1)]
test_prob.to_csv('../sub_prob/test_prob_fasttext_cv_%s.csv'%score,index=None)
