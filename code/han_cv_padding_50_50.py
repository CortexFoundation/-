...
def get_model():
    ...
    bigru_sent = Bidirectional(CuDNNGRU(128))(bigru_sent)
...
train_prob.to_csv('../sub_prob/train_prob_avgbigru_cv_%s.csv' % score, index=None)
# 保存预测概率文件
test_prob = pd.DataFrame(test_pred)
test_prob.columns = ["class_prob_%s" % i for i in range(1, test_pred.shape[1] + 1)]
test_prob.to_csv('../sub_prob/test_prob_avgbigru_cv_%s.csv' % score, index=None)
#生成提交结果
...
test_pred[["id","class"]].to_csv('../sub/avgbigru_cv_%s.csv'%score,index=None)
