...
model1 = SelectKBest(chi2, k=100000)   #8000-0.698,100000-0.690
...
def stacking(clf,train_x,train_y,test_x,clf_name,class_num=1):
    ...
    for i,(train_index,test_index) in enumerate(kf):
        ...
        if clf_name=="rf":
            model = RandomForestClassifier(100,random_state=2018,n_jobs=15)
            model.fit(tr_x, tr_y)
            pre = model.predict_proba(te_x)
            pred=model.predict_proba(test_x)
...
train_prob.to_csv('../sub_prob/train_prob_rf_select_cv_%s.csv'%score,index=None)
#保存预测概率文件
test_prob=pd.DataFrame(lgb_test)
test_prob.columns=["class_prob_%s"%i for i in range(1,lgb_test.shape[1]+1)]
test_prob["id"]=list(test_id["id"])
test_prob.to_csv('../sub_prob/test_prob_rf_select_cv_%s.csv'%score,index=None)
...
test_pred[["id","class"]].to_csv('../sub/sub_rf_select_cv_%s.csv'%score,index=None)
t2=time.time()
