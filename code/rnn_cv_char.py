...
t1 = time.time()
###################################################################################################################################
model = word2vec.Word2Vec.load("daguan_char_40.w2v")
...
for ii, (idx_train, idx_val) in enumerate(skf):
    ...
    history = model.fit(X_train_tr, y_tr, batch_size=128, epochs=2, verbose=1, validation_data=(X_train_te, y_te),
...
train_prob.to_csv('../sub_prob/train_prob_rnn_char_cv_%s.csv' % score, index=None)
# 保存预测概率文件
test_prob = pd.DataFrame(test_pred)
test_prob.columns = ["class_prob_%s" % i for i in range(1, test_pred.shape[1] + 1)]
test_prob.to_csv('../sub_prob/test_prob_rnn_char_cv_%s.csv' % score, index=None)
#生成提交结果
...
test_pred[["id","class"]].to_csv('../sub/rnn_char_cv_%s.csv'%score,index=None)
