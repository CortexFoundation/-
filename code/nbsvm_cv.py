...
class NbSvmClassifier(BaseEstimator, ClassifierMixin):
    ...
    def fit(self, x, y):
        ...
        def pr(x, y_i, y):
            p = x[y==y_i].sum(0)
            return (p+1.0) / ((y==y_i).sum()+1.0)

        self._r = sparse.csr_matrix(np.log(pr(x,1.0,y) / pr(x,0.0,y)))
...
def stacking(clf,train_x,train_y,test_x,clf_name,class_num=1):
    ...
    for i,(train_index,test_index) in enumerate(kf):
        ...
        if clf_name=="nbsvm":
            model = NbSvmClassifier(C=4.0, dual=False)
            model.fit(tr_x, tr_y)
            pre = model.predict_proba(te_x)
            pred=model.predict_proba(test_x)
...
def nbsvm(x_train, y_train, x_valid):
    xgb_train, xgb_test,cv_scores = stacking(lightgbm, x_train, y_train, x_valid,"nbsvm",19)
    return xgb_train, xgb_test,cv_scores
...
test_prob.to_csv('../sub_prob/test_prob_nblr_cv_%s.csv'%score,index=None)
...
test_pred[["id","class"]].to_csv('../sub/sub_nblr_cv_%s.csv'%score,index=None)
t2=time.time()
