...
tr.columns=["plantsgo_%s"%i for i in range(train_x.shape[1])]
tr["id"]=list(train_id["id"])
te=pd.DataFrame(test_x)
te.columns=["plantsgo_%s"%i for i in range(test_x.shape[1])]
te["id"]=list(test_id["id"])
tr.to_csv("plantsgo_train.csv",index=None)
te.to_csv("plantsgo_test.csv",index=None)
"""
...
def stacking(clf,train_x,train_y,test_x,clf_name,class_num=1):
    ...
    for i,(train_index,test_index) in enumerate(kf):
        ...
        if clf_name == "xgb":
            train_matrix = clf.DMatrix(tr_x, label=tr_y, missing=np.nan)
            test_matrix = clf.DMatrix(te_x, label=te_y, missing=np.nan)
            z = clf.DMatrix(test_x, label=te_y, missing=np.nan)
            ...
            pre = model.predict(test_matrix, ntree_limit=model.best_ntree_limit).reshape((te_x.shape[0], class_num))
            pred = model.predict(z, ntree_limit=model.best_ntree_limit).reshape((test_x.shape[0], class_num))
...
def xgb(x_train, y_train, x_valid):
    xgb_train, xgb_test, cv_scores = stacking(xgboost, x_train, y_train, x_valid, "xgb", 19)
    return xgb_train, xgb_test, cv_scores
...
train_pred.to_csv("../sub/train_prob_lgb_v2.csv",index=None)

test_pred=pd.DataFrame(lgb_test)
test_pred.columns=["class_%s"%i for i in range(19)]
test_pred.to_csv("../sub/test_prob_lgb_v2.csv",index=None)
#生成提交结果
...
test_pred[["id","class"]].to_csv('../sub/sub_stacking_%s_%s.csv'%(score,loss_score),index=None)
t2=time.time()
