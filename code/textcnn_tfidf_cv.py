...
X_train_tfidf=trn_term_doc.tocsr()
X_test_tfidf=test_term_doc.tocsr()
...
def get_model():
    ...
    inp_tfidf = Input(shape=(X_train_tfidf.shape[1],))
    inp_dense=Dropout(0.5)(inp_tfidf)
    inp_dense=Dense(160)(inp_dense)
    """
    textcnn_list=[]
    for i in [2,3,4,5,6]:
    ...
    textcnn_list = [p1, p2, p3, p4, p5,inp_dense]
    ...
    model = Model(inputs=[inp,inp_tfidf], outputs=output)
...
for ii, (idx_train, idx_val) in enumerate(skf):
    ...
    X_train_tfidf_tr = X_train_tfidf[idx_train]
    X_train_tfidf_te = X_train_tfidf[idx_val]
    ...
    history = model.fit([X_train_tr,X_train_tfidf_tr], y_tr, batch_size=128, epochs=10, verbose=1, validation_data=([X_train_te,X_train_tfidf_te], y_te),
                        callbacks=[early_stop,check_point])

    model.load_weights('cate_model.hdf5')
    preds_te = model.predict([X_train_te,X_train_tfidf_te])
    ...
    preds = model.predict([X_test,X_test_tfidf])
...
test_pred[["id","class"]].to_csv('../sub/textcnn_cv_%s.csv'%score,index=None)
