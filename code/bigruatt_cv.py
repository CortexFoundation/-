...
###################################################################################################################################
# 建立模型
from model_utils import AttentivePoolingLayer,Attention,AttentionWithContext
def get_model():
...
train_prob.to_csv('../sub_prob/train_prob_bigruatt_cv_%s.csv' % score, index=None)
# 保存预测概率文件
test_prob = pd.DataFrame(test_pred)
test_prob.columns = ["class_prob_%s" % i for i in range(1, test_pred.shape[1] + 1)]
test_prob.to_csv('../sub_prob/test_prob_bigruatt_cv_%s.csv' % score, index=None)
