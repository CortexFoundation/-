...
train_prob.to_csv('../sub_prob/train_prob_lr_cv_all_%s.csv'%score,index=None)
#保存预测概率文件
test_prob=pd.DataFrame(lgb_test)
test_prob.columns=["class_prob_%s"%i for i in range(1,lgb_test.shape[1]+1)]
test_prob["id"]=list(test_id["id"])
test_prob.to_csv('../sub_prob/test_prob_lr_cv_all_%s.csv'%score,index=None)
...
test_pred[["id","class"]].to_csv('../sub/sub_lr_cv_all_%s.csv'%score,index=None)
t2=time.time()
