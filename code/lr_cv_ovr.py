...
def stacking(clf,train_x,train_y,test_x,clf_name,class_num=1):
    ...
    for i,(train_index,test_index) in enumerate(kf):
        ...
        train[test_index]=pre[:,1].reshape((-1,1))

        test_pre[i, :]= pred[:,1].reshape((-1,1))
...
for ovr in range(19):
    ...
    train_prob.to_csv('../sub_prob/train_prob_lr_cv_ovr_%s_%s.csv'%(ovr,score),index=None)
    #保存预测概率文件
    test_prob=pd.DataFrame(lgb_test)
    test_prob.columns=["svm_ovr_%s"%ovr]
    test_prob.to_csv('../sub_prob/test_prob_lr_cv_ovr_%s_%s.csv'%(ovr,score),index=None)
