...
def stacking(clf,train_x,train_y,test_x,clf_name,class_num=1):
    ...
    for i,(train_index,test_index) in enumerate(kf):
        ...
        if clf_name=="roc":
            ...
            model.fit(tr_x, tr_y)
            pre = model.predict(te_x).reshape((te_x.shape[0],1))
            pred=model.predict(test_x).reshape((test_x.shape[0],1))

        if clf_name=="knn":
            model = KNeighborsClassifier(100,n_jobs=-1)
            model.fit(tr_x, tr_y)
            pre = model.predict(te_x).reshape((te_x.shape[0],1))
            pred=model.predict(test_x).reshape((test_x.shape[0],1))
...
def nb(x_train, y_train, x_valid):
    xgb_train, xgb_test,cv_scores = stacking(lightgbm, x_train, y_train, x_valid,"nb",19)
    return xgb_train, xgb_test,cv_scores
...
def knn(x_train, y_train, x_valid):
    xgb_train, xgb_test,cv_scores = stacking(lightgbm, x_train, y_train, x_valid,"knn",1)
    return xgb_train, xgb_test,cv_scores
...
lgb_train, lgb_test,m=knn(train_x, train_y, test_x)
...
train_prob.to_csv('../sub_prob/train_prob_knn100_select_cv_%s.csv'%score,index=None)
#保存预测概率文件
test_prob=pd.DataFrame(lgb_test)
test_prob.columns=["class_prob_%s"%i for i in range(1,lgb_test.shape[1]+1)]
test_prob["id"]=list(test_id["id"])
test_prob.to_csv('../sub_prob/test_prob_knn100_select_cv_%s.csv'%score,index=None)
