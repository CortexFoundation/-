...
train_pred.to_csv("../sub/train_prob_lr.csv",index=None)

test_pred=pd.DataFrame(lgb_test)
test_pred.columns=["class_%s"%i for i in range(19)]
test_pred.to_csv("../sub/test_prob_lr.csv",index=None)
#生成提交结果
...
test_pred[["id","class"]].to_csv('../sub/sub_stacking_lr_%s_%s.csv'%(score,loss_score),index=None)
t2=time.time()
