...
with open("score_cv.txt", "a") as f:
    f.write("%s now score is:" % "att" + str(score_cv_list) + "\n")
...
train_prob.to_csv('../sub_prob/train_prob_att_cv_%s.csv' % score, index=None)
# 保存预测概率文件
test_prob = pd.DataFrame(test_pred)
test_prob.columns = ["class_prob_%s" % i for i in range(1, test_pred.shape[1] + 1)]
test_prob.to_csv('../sub_prob/test_prob_att_cv_%s.csv' % score, index=None)
#生成提交结果
...
test_pred[["id","class"]].to_csv('../sub/att_cv_%s.csv'%score,index=None)
