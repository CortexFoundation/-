...
for ovr in range(19):
    ...
    lgb_train, lgb_test,m=ridge(train_x, train_y, test_x)
    ...
    train_prob.to_csv('../sub_prob/train_prob_ridge_cv_ovr_%s_%s.csv'%(ovr,score),index=None)
    #保存预测概率文件
    test_prob=pd.DataFrame(lgb_test)
    test_prob.columns=["svm_ovr_%s"%ovr]
    test_prob.to_csv('../sub_prob/test_prob_ridge_cv_ovr_%s_%s.csv'%(ovr,score),index=None)
