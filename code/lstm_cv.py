...
from keras.layers import LSTM, concatenate, Conv2D,SpatialDropout1D,Bidirectional,CuDNNGRU,CuDNNLSTM
...
def get_model():
    ...
    emb = Embedding(nb_words, EMBEDDING_DIM, input_length=MAX_SEQUENCE_LENGTH, weights=[embedding_matrix],
                    trainable=False)(inp)
    lstm=Bidirectional(CuDNNLSTM(128,return_sequences=True))(emb)
    lstm=Attention(2500)(lstm)

    fc1 = Dense(256, activation='relu')(lstm)
...
for ii, (idx_train, idx_val) in enumerate(skf):
    ...
    history = model.fit(X_train_tr, y_tr, batch_size=128, epochs=20, verbose=1, validation_data=(X_train_te, y_te),
...
train_prob.to_csv('../sub_prob/train_prob_lstm_cv_%s.csv' % score, index=None)
# 保存预测概率文件
test_prob = pd.DataFrame(test_pred)
test_prob.columns = ["class_prob_%s" % i for i in range(1, test_pred.shape[1] + 1)]
test_prob.to_csv('../sub_prob/test_prob_lstm_cv_%s.csv' % score, index=None)
#生成提交结果
...
test_pred[["id","class"]].to_csv('../sub/lstm_cv_%s.csv'%score,index=None)
