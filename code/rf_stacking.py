...
train_x = np.concatenate([train_x, train_textcnn,train_rnn,train_att,train_mlp,train_fasttext], axis=1)
test_x = np.concatenate([test_x, test_textcnn,test_rnn,test_att,test_mlp,test_fasttext], axis=1)
#"""
print(train_x.shape)
print(test_x.shape)
...
def stacking(clf,train_x,train_y,test_x,clf_name,class_num=1):
    ...
    for i,(train_index,test_index) in enumerate(kf):
        ...
        if clf_name=="rf":
            model = RandomForestClassifier(10000,random_state=2018,n_jobs=15)
            model.fit(tr_x, tr_y)
            pre = model.predict_proba(te_x)
            pred=model.predict_proba(test_x)
...
def rf(x_train, y_train, x_valid):
    xgb_train, xgb_test,cv_scores = stacking(lightgbm, x_train, y_train, x_valid,"rf",19)
    return xgb_train, xgb_test,cv_scores
...
lgb_train, lgb_test,m=rf(train_x, train_y, test_x)
...
train_pred.to_csv("../sub/train_prob_rf.csv",index=None)

test_pred=pd.DataFrame(lgb_test)
test_pred.columns=["class_%s"%i for i in range(19)]
test_pred.to_csv("../sub/test_prob_rf.csv",index=None)
#生成提交结果
...
test_pred[["id","class"]].to_csv('../sub/sub_stacking_rf_%s_%s.csv'%(score,loss_score),index=None)
t2=time.time()
