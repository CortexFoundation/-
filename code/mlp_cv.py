...
for ii,(idx_train, idx_val) in enumerate(skf):
    ...
    history = model.fit(X_train_tr, y_tr, batch_size=1024, epochs=3, verbose=1, validation_data=(X_train_te,y_te),
...
train_prob.to_csv('../sub_prob/train_prob_mlp_cv_word_%s.csv'%score,index=None)
#保存预测概率文件
test_prob=pd.DataFrame(test_pred)
test_prob.columns=["class_prob_%s"%i for i in range(1,test_pred.shape[1]+1)]
test_prob.to_csv('../sub_prob/test_prob_mlp_cv_word_%s.csv'%score,index=None)
