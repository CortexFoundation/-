...
###################################################################################################################################
# 建立模型
from model_utils import AttentivePoolingLayer,Capsule
def get_model():
    Routings = 5
    Num_capsule = 10
    Dim_capsule = 16
    ...
    capsule = Capsule(num_capsule=Num_capsule, dim_capsule=Dim_capsule, routings=Routings,
                      share_weights=True)(gru)

    capsule = Flatten()(capsule)

    fc1 = Dense(256, activation='relu')(capsule)
...
for ii, (idx_train, idx_val) in enumerate(skf):
    ...
    history = model.fit(X_train_tr, y_tr, batch_size=128, epochs=50, verbose=1, validation_data=(X_train_te, y_te),
...
train_prob.to_csv('../sub_prob/train_prob_cap_cv_%s.csv' % score, index=None)
# 保存预测概率文件
test_prob = pd.DataFrame(test_pred)
test_prob.columns = ["class_prob_%s" % i for i in range(1, test_pred.shape[1] + 1)]
test_prob.to_csv('../sub_prob/test_prob_cap_cv_%s.csv' % score, index=None)
#生成提交结果
...
test_pred[["id","class"]].to_csv('../sub/cap_cv_%s.csv'%score,index=None)
