...
for ovr in range(19):
    ...
    train_prob.to_csv('../sub_prob/train_prob_roc_cv_ovr_%s_%s.csv'%(ovr,score),index=None)
    #保存预测概率文件
    test_prob=pd.DataFrame(lgb_test)
    test_prob.columns=["svm_ovr_%s"%ovr]
    test_prob.to_csv('../sub_prob/test_prob_roc_cv_ovr_%s_%s.csv'%(ovr,score),index=None)
